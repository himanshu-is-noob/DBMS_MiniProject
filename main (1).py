...
from PIL import Image, ImageTk  # To handle the image
...
class Management:
    def __init__(self, root):
        ...
        self.bg_image = Image.open("c:/Users/ADMIN/OneDrive/Desktop/dbms project/AddStudentFrameBg.png")  # Path to the uploaded image
        self.bg_image = self.bg_image.resize((740, 540), Image.LANCZOS)  # Resize to fit frame  # Resize to fit frame
        ...
        self.bg_label = Label(self.frame_1, image=self.bg_photo)
        ...
        style.configure('TButton',
                        font=(cs.font_1, 12),
                        padding=10,
                        background=cs.color_2,
                        foreground=cs.color_3)
        style.map('TButton',
                  background=[('active', '#4CAF50'), ('pressed', '#3e8e41')],
                  foreground=[('active', 'black')])

        self.add_book = ttk.Button(self.frame_2, text='Add Book', command=self.AddNewBook)
        self.add_book.grid(row=0, column=0, padx=20, pady=10)

        self.issue_book = ttk.Button(self.frame_2, text='Issue Book', command=self.GetData_for_IssueBook)
        self.issue_book.grid(row=1, column=0, padx=20, pady=10)

        self.return_book = ttk.Button(self.frame_2, text='Return Book', command=self.ReturnBook)
        self.return_book.grid(row=2, column=0, padx=20, pady=10)

        self.all_book = ttk.Button(self.frame_2, text='All Books', command=self.ShowBooks)
        self.all_book.grid(row=3, column=0, padx=20, pady=10)

        self.search_book = ttk.Button(self.frame_2, text='Search Book', command=self.GetBookNametoSearch)
        self.search_book.grid(row=4, column=0, padx=20, pady=10)

        self.all_borrow_records = ttk.Button(self.frame_2, text='Book Holders', command=self.AllBorrowRecords)
        self.all_borrow_records.grid(row=5, column=0, padx=20, pady=10)

        self.clear = ttk.Button(self.frame_2, text='Clear Screen', command=self.ClearScreen)
        self.clear.grid(row=6, column=0, padx=20, pady=10)

        self.exit = ttk.Button(self.frame_2, text='Exit', command=self.Exit)
        self.exit.grid(row=7, column=0, padx=20, pady=10)



      

        #self.add_book = Button(self.frame_2, text='Add Book', font=(cs.font_1, 12), bd=2, command=self.AddNewBook,cursor="hand2", bg=cs.color_2,fg=cs.color_3).grid(row=0, column=0, padx=20, pady=20)
        #self.issue_book = Button(self.frame_2, text='Issue Book', font=(cs.font_1, 12), bd=2, command=self.GetData_for_IssueBook,cursor="hand2", bg=cs.color_7,fg=cs.color_3).grid(row=1, column=0, padx=20, pady=20)
        #self.return_book = Button(self.frame_2, text='Return Book', font=(cs.font_1, 12), bd=2, command=self.ReturnBook,cursor="hand2", bg=cs.color_6,fg=cs.color_3).grid(row=2, column=0, padx=20, pady=20)
        #self.all_book = Button(self.frame_2, text='All Books', font=(cs.font_1, 12), bd=2, command=self.ShowBooks,cursor="hand2", bg=cs.color_2,fg=cs.color_3).grid(row=3, column=0, padx=20, pady=20)
        
        
        #self.search_book = Button(self.frame_2, text='Search Book', font=(cs.font_1, 12), bd=2, command=self.GetBookNametoSearch,cursor="hand2", bg=cs.color_2,fg=cs.color_3).place(x=190,y=20,width=100)
        #self.all_borrow_records = Button(self.frame_2, text='Book Holders', font=(cs.font_1, 12), bd=2, command=self.AllBorrowRecords, cursor="hand2", bg=cs.color_2,fg=cs.color_3).place(x=190,y=92,width=100)
        #self.clear = Button(self.frame_2, text='Clear Screen', font=(cs.font_1, 12), bd=2, command=self.ClearScreen,cursor="hand2", bg=cs.color_2,fg=cs.color_3).place(x=190,y=165,width=100)
        #self.exit = Button(self.frame_2, text='Exit', font=(cs.font_1, 12), bd=2, command=self.Exit,cursor="hand2", bg=cs.color_2,fg=cs.color_3).place(x=190,y=240,width=100)
    ...
    def OnSelectedforReturn(self, a):
        ...
        self.dlt_record = ttk.Button(self.frame_3, text='Delete', command=self.ReturningBook).place(x=50,y=0,width=100)
        self.borrow_again = ttk.Button(self.frame_3, text='Issue Again',command=self.IssueAgain).place(x=180,y=0,width=100)
    ...
    def GetData_for_IssueBook(self):
        ...
        # Create a labeled entry with placeholder functionality
         def create_entry_with_placeholder(label_text, placeholder, x_label, y_label):
            label = Label(self.frame_1, text=label_text, font=(cs.font_2, 15, "bold"), bg=cs.color_1)
            label.place(x=x_label, y=y_label)

            entry = Entry(self.frame_1, bg=cs.color_4, fg='lightgrey', font=(cs.font_2, 12))  # Light grey for placeholder
            entry.place(x=x_label, y=y_label + 30, width=200)
            entry.insert(0, placeholder)

            # Bind focus events to manage placeholder behavior
            entry.bind("<FocusIn>", lambda event: self.on_entry_click(entry, placeholder))
            entry.bind("<FocusOut>", lambda event: self.on_focusout(entry, placeholder))

            entry.config(borderwidth=2, relief="groove")  # Add stylish border
            return entry

        # Create entries with labels and placeholders
         self.book_id_entry = create_entry_with_placeholder("Book Id", "Enter Book ID", 130, 30)
         self.book_name_entry = create_entry_with_placeholder("Book Name", "Enter Book Name", 400, 30)
         self.stu_roll_entry = create_entry_with_placeholder("Student Roll", "Enter Roll Number", 130, 100)
         self.stu_name_entry = create_entry_with_placeholder("Student Name", "Enter Student Name", 400, 100)
         self.course_entry = create_entry_with_placeholder("Course", "Enter Course Name", 130, 170)
         self.subject_entry = create_entry_with_placeholder("Subject", "Enter Subject", 400, 170)
         self.issue_date_entry = create_entry_with_placeholder("Issue Date", "Enter Issue Date", 130, 240)
         self.return_date_entry = create_entry_with_placeholder("Return Date", "Enter Return Date", 400, 240)

        # Submit Button
         self.submit_bt_1 = Button(self.frame_1, text='Submit', font=(cs.font_1, 12), bd=2, 
                                   command=self.Submit_borrow_data, cursor="hand2", 
                                   bg=cs.color_2, fg=cs.color_3)
         self.submit_bt_1.place(x=300, y=320, width=100)

    def on_entry_click(self, entry, placeholder):
        """Clear the placeholder text when focused."""
        if entry.get() == placeholder:
            entry.delete(0, "end")  # Delete current text
            entry.config(fg=cs.color_3)  # Change text color to normal

    def on_focusout(self, entry, placeholder):
        """Restore the placeholder text when focus is lost."""
        if entry.get() == "":
            entry.insert(0, placeholder)  # Reinsert placeholder
            entry.config(fg='lightgrey')  # Change text color to light grey
